...
data Check_for_CanDoGenerics1 = CCDG1
  { _ccdg1_hasParam :: Bool       -- does the parameter of interest occurs in
                                  -- this type?
  , _ccdg1_numTyAps :: Int        -- Number of times a type constructor has been
...
canDoGenerics1 rep_tc =
  ...
  where
    ...
    bmzero      = CCDG1 False 0 IsValid
    bmbad con s = CCDG1 True 0 $ NotValid $ bad con s
    bmplus (CCDG1 b1 n1 m1) (CCDG1 b2 n2 m2) = CCDG1 (b1 || b2) (n1 + n2) (m1 `andValid` m2)
    bmap (CCDG1 b n m) = CCDG1 b (if b then n+1 else n) m
    ...
    too_many_aps = text "applies the parameter but the application"
                $$ text "isn't of the the form (p (f p)) or (p c)"
